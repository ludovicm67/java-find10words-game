...
public class Game {
	...
	public void verify(Word word, Player p){
	  if(word.is_word()){
			if(this.verifyFindWord(word, p)) System.out.println("Sorry, the word has been already found.");
			else if(!(this.verifyLetters(word.getWord()))) System.out.println("Sorry, some letters aren't in the pot.");
			else{
  ...
  public boolean verifyFindWord(Word word, Player p){
    for(Word str : p.getWords()){
      if(word.getWord().equals(str.getWord())){
        return true;
      }
    }
		return false;
